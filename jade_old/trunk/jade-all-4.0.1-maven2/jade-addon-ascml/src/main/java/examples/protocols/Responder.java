...
* @version $Date: 2004/04/29 20:49:18 $ $Revision: 1.2 $
**/


public class Responder extends Agent implements jade.tools.ascml.launcher.DebugableInterface {
    ...
    */
	...
	protected ACLMessage prepareResponse(ACLMessage request) throws RefuseException, NotUnderstoodException {
	    ...
	    if (debug) System.out.println( myAgent.getLocalName() +  " --> is sending "+(response==null?"no":(response.getPerformative()==ACLMessage.SUBSCRIBE?"an out-of-sequence":ACLMessage.getPerformative(response.getPerformative())))+ " response to the protocol initiator." );
	    return response;	
	}
	...
	protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) throws FailureException {
	    ...
	    if (debug) System.out.println( myAgent.getLocalName() +  " --> is sending "+(resNot==null?"no":(resNot.getPerformative()==ACLMessage.SUBSCRIBE?"an out-of-sequence":ACLMessage.getPerformative(resNot.getPerformative())))+ " result notification to the protocol initiator." );
	    return resNot;
	}

 ...
 */
public class SIP_TestActionDone extends NotificationSIPAdapter {
   
    //private static Formula Pattern = SL.formula("(B ??myself (exists ??e (done (; ??action ??e))))");
    private static Formula Pattern = SL.formula("(B ??myself (exists ??e (done (; (action ??agent ??action) ??e))))");

    private SemanticCapabilities get_sc()
    { return this.myCapabilities;}
    
    public SIP_TestActionDone(SemanticCapabilities capabilities) {
    ...
    private boolean
    action_is_done_action(Term t)
    {
        Formula f = SL.formula("(action_done ??action)").instantiate("action", t);
...
//    {
//        System.out.println("++++++++++++++ " + sr.getSemanticInterpretationPrincipleIndex() + " - " + sr.getSLRepresentation());
//        return super.apply(sr);
//    }
            ...
            SemanticRepresentation sr)
    {
        ...
        {
            if (action_is_done_action(applyResult.term("action")))
            {
                System.out.println("[OK] :ActionDone:" + applyResult.term("action").toString());   
            }
            else
            {
                System.out.println("#KO# :ActionDone:" + applyResult.term("action").toString());   
            }
        }
    } // End of apply/1

...
/**
   This agent implements the logic of the chat client running on the 
   user terminal. User interactions are handled by the ChatGui in a
   terminal-dependent way.
   The ChatClientAgent performs 3 types of behaviours:
   - ParticipantsManager. A CyclicBehaviour that keeps the list of 
   participants up to date on the basis of the information received
   from the ChatManagerAgent. This behaviour is also in charge of 
   subscribing as a participant to the ChatManagerAgent.
   - ChatListener. A CyclicBehaviour that handles messages from other
   chat participants.
   - ChatSpeaker. A OneShotBehaviour that sends a message conveying a
   sentence written by the user to other chat participants. 
